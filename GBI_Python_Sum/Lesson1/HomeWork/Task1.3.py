# Вы пользуетесь общественным транспортом? Вероятно, вы расплачивались
# за проезд и получали билет с номером. Счастливым билетом называют
# такой билет с шестизначным номером, где сумма первых трех цифр равна
# сумме последних трех. Т.е. билет с номером 385916 – счастливый, т.к.
# 3+8+5=9+1+6. Вам требуется написать программу, которая проверяет
# счастливость билета.

number = int(input('Введите шестизначное число билета: '))
sum1 = (number//1000)//100+(number//1000)%100//10+(number//1000)%100%10 
sum2 = (number%1000)//100+(number%1000)%100//10+(number%1000)%100%10
if sum1 == sum2:
    print('Ура! Это счастливый билет')
else:
    print('Жаль! Не счастливый билет')
